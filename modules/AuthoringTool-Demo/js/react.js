 ...
 * All rights reserved.
 *
 * This source code is licensed under the BSD-css license found in the
 ...
 * @param {string} key css name, eg: transitionDuration
 * @return {string} css name prefixed with `prefix`, properly camelCased, eg:
...
/**
 * Support css names that may come passed in prefixed by adding permutations
 * of vendor prefixes.
 */
...
/**
 * Most css properties can be unset by doing .css[prop] = '' but IE8
 * doesn't like doing that with shorthand properties so for the properties that
 * IE8 breaks on, which are listed here, we instead unset each of the
 * individual properties. See http://bugs.jquery.com/ticket/12385.
 * The 4-value 'clock' properties like margin, padding, border-width seem to
 * behave without any problems. Curiously, list-css works too without any
 * special prodding.
 */
...
if (ExecutionEnvironment.canUseDOM) {
  ...
  try {
    // IE8 throws "Invalid argument." if resetting shorthand css properties.
    tempStyle.font = '';
  } catch (e) {
...
if ("development" !== 'production') {
  ...
  // css values shouldn't contain a semicolon
  ...
  var warnHyphenatedStyleName = function (name) {
    ...
    "development" !== 'production' ? warning(false, 'Unsupported css property %s. Did you mean %s?', name, camelizeStyleName(name)) : undefined;
  };
  ...
  var warnBadVendoredStyleName = function (name) {
    ...
    warnedStyleNames[name] = true;
    "development" !== 'production' ? warning(false, 'Unsupported vendor-prefixed css property %s. Did you mean %s?', name, name.charAt(0).toUpperCase() + name.slice(1)) : undefined;
  };
...
var CSSPropertyOperations = {
  ...
  /**
   * Serializes a mapping of css properties for use as inline styles:
   *
   ...
   * '' (empty string), the corresponding css property will be unset.
   *
...
function checkAndWarnForMutatedStyle(style1, style2, component) {
  ...
  "development" !== 'production' ? warning(false, '`%s` was passed a css object that has previously been mutated. ' + 'Mutating `css` is deprecated. Consider cloning it beforehand. Check ' + 'the `render` %s. Previous css: %s. Mutated css: %s.', componentName, owner ? 'of `' + ownerName + '`' : 'using <' + componentName + '>', friendlyStringify(style1), friendlyStringify(style2)) : undefined;
}
...
function assertValidProps(component, props) {
  ...
  !(props.style == null || typeof props.style === 'object') ? "development" !== 'production' ? invariant(false, 'The `css` prop expects a mapping from css properties to values, ' + 'not a string. For example, css={{marginRight: spacing + \'em\'}} when ' + 'using JSX.%s', getDeclarationErrorAddendum(component)) : invariant(false) : undefined;
}
 ...
 *
 * The `css` property functions differently from the DOM API. It accepts an
 * object mapping of css properties to values.
 *
    ...
    for (var propKey in props) {
      ...
      if (registrationNameModules.hasOwnProperty(propKey)) {
        ...
        if (propKey === STYLE) {
          if (propValue) {
            if ("development" !== 'production') {
              // See `_updateDOMProperties`. css block
...
var INVALID_PROPERTY_ERRORS = {
  ...
  style: '`css` must be set using `updateStylesByID()`.'
};
...
/**
 * Convert a value into the proper css writable value. The css name `name`
 * should be logical (no hyphens), as specified
 * in `CSSProperty.isUnitlessNumber`.
 *
 * @param {string} name CSS property name such as `topMargin`.
 * @param {*} value CSS property value such as `10px`.
 * @return {string} Normalized css value with dimensions applied.
 */
 ...
 *   < "background-color"
 *
 * For CSS css names, use `hyphenateStyleName` instead which works properly
 ...
 *
 * Provide sprintf-css format (only %s is supported) and arguments
