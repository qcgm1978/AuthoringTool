    ...
    var isMac = w.navigator.platform == 'MacIntel',
        ...
        bubble = {
            ...
            updatePos: function(editor, elem) {
                var sel = w.getSelection(),
                    ...
                    pos = {
                        x: boundary.left + $(document).scrollLeft() - editor.offset().left - bubbleWidth/2 ,
                        y: boundary.top + $(document).scrollTop() - editor.offset().top - bubbleHeight/2 -20
                    };
                /*console.log(boundary.top + "/" + editor.offset().top + "/" +  (bubbleHeight/2 -20)  + " / " + $(document).scrollTop());*/
        ...
        actions = {
            bindEvents: function(elem) {
                /*elem.off("keydown keyup focus paste mousedown mouseup mousemove blur");*/
                elem.off("keydown");
                elem.off("keyup");
                elem.off("focus");
                elem.off("paste");
            ...
            setPlaceholder: function(e) {
                ...
                if (/^\s*$/.test($(this).text())) {
                    ...
                    var $plc =  $(this).find('.placeholder');
                    if ($plc.length > 0 && $.trim($plc.text()).length > 0 && ($plc.text() == options.placeholder)) {
